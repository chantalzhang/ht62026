...
from qnx_rocky_vision.landmark_viewer import OpenCVCamera, QnxCameraSource
from qnx_rocky_vision.rps import counter_move, result_text
...
class Game:
    ...
    def run_vision(self):
        if self.args.source == "opencv":
            camera = OpenCVCamera(self.args.camera_index, self.args.width, self.args.height)
        else:
            camera = QnxCameraSource(self.args.width, self.args.height)
        with mp_hands.Hands(
...
def parse_args():
    ...
    parser.add_argument("--source", choices=("qnx", "opencv"), default="qnx")
